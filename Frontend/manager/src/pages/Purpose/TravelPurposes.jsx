...
import { Target, Search, PlusIcon, PencilIcon, TrashIcon, BriefcaseIcon } from "lucide-react"
import {
  getTravelPurposes,
  createTravelPurpose,
  updateTravelPurpose,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Travel Purposes</h1>
          <p className="text-gray-600">Manage travel purpose categories and types</p>
        </div>
        <button
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{purposes.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{purposes.filter((p) => p.status === "Active").length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{purposes.reduce((sum, p) => sum + p.hotelCount, 0)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div className="col-span-full text-center text-gray-500 py-8">No travel purposes found</div>
        ) : (
          purposes.map((purpose) => (
            <div key={purpose.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="mb-4">
                <h3 className="text-lg font-bold text-gray-900 mb-1">{purpose.name}</h3>
                <p className="text-sm text-gray-600">{purpose.description}</p>
              </div>
              <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                <div className="text-sm text-gray-600">
                  <span className="font-medium text-gray-900">{purpose.hotelCount}</span> hotels
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => handleOpenModal(purpose)} className="text-primary hover:text-primary-dark p-1">
                    <PencilIcon className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDeleteClick(purpose)} className="text-red-600 hover:text-red-800 p-1">
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Delete Travel Purpose">
        <div className="space-y-4">
          <p className="text-gray-600">
            Are you sure you want to delete <strong>{selectedPurpose?.name}</strong>? This action cannot be undone.
          </p>
          <div className="flex gap-3 pt-4">
