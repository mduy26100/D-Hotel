...
import { DoorOpen, Search, PlusIcon, PencilIcon, TrashIcon, CheckCircle, XCircle } from "lucide-react"
import { getRoomPurposes, createRoomPurpose, updateRoomPurpose, deleteRoomPurpose } from "../../api/mock/roomPurposes"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Room Purposes</h1>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{roomPurposes.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{roomPurposes.filter((rp) => rp.isAvailable).length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                  ...
                  <tr key={rp.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-sm font-medium text-gray-900">{rp.roomNumber}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{rp.purposeName}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded flex items-center gap-1 w-fit ${
                          rp.isAvailable ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                        }`}
                      >
                        {rp.isAvailable ? (
                          <>
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-600 max-w-xs">{rp.notes}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded ${
                          rp.status === "Active" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                        }`}
                      >
                        {rp.status}
                      </span>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Delete Room Purpose">
        <div className="space-y-4">
          <p className="text-gray-600">
            Are you sure you want to delete the purpose assignment for room <strong>{selectedRP?.roomNumber}</strong>?
